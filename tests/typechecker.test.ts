...
function check(src: string) {
  const ast = parse(lex(src, "t.fe"), src, "t.fe");
  return typecheck(ast, src, "t.fe");
}
...
`;
  ...
  it("requires a return on every path", () => {
    ...
    expect(e.code).toBe("FER208");
  });

  it("rejects assigning to an immutable binding", () => {
    const e = err(`def main() -> i32:\n    let x: i32 = 1\n    x = 2\n    return x\n`);
    expect(e.code).toBe("FER217");
...
`;
  ...
  it("rejects moving a borrowed value", () => {
    ...
    return 0
`;
    expect(err(src).code).toBe("FER219");
  });
  ...
  it("rejects mutating while borrowed", () => {
    ...
    return 0
`;
    expect(err(src).code).toBe("FER220");
  });
...
def main() -> i32:
    ...
    return p.y
`;
    expect(err(src).code).toBe("FER209");
  });
});
