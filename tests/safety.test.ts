...
function error(src: string, code: string) {
  try { compileSource(src, "safety.fe"); throw new Error("expected diagnostic"); }
...
describe("safety regressions", () => {
  it.each([
    ...
    ["statement separator", main("    let x: i32 = 1 return x"), "FER102"],
...
describe("native correctness", () => {
  it.each([
    ...
    const dir = mkdtempSync(join(tmpdir(), "ferra-safety-"));
    try {
      const file = join(dir, "test.fe"), out = join(dir, "test");
