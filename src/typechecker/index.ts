...
export type FnInfo = { name: string; params: { name: string; type: LycaType }[]; ret: LycaType; pythonModule?: string };

export type CheckedProgram = {
  ast: Program;
  structs: Map<string, StructInfo>;
  fns: Map<string, FnInfo>;
  exprTypes: Map<Expr, LycaType>;
  types: Map<TypeAst, LycaType>;
...
class Typechecker {
  ...
  stmtBorrows: VarInfo[] = [];
  exprTypes = new Map<Expr, LycaType>();
  types = new Map<TypeAst, LycaType>();
  loopDepth = 0;
  ...
  run(): CheckedProgram {
    ...
    const visit = (t: LycaType): void => {
      if (t.kind === "ref") this.err("LYC224", "references cannot be stored in aggregates", this.ast.span);
      if (t.kind === "array") visit(t.element);
      if (t.kind !== "struct" || done.has(t.name)) return;
      if (visiting.has(t.name)) this.err("LYC228", "recursive value layout requires indirection", this.ast.span);
      visiting.add(t.name);
    ...
    for (const d of this.ast.decls) {
      ...
      if (fn.ret.kind === "ref") this.err("LYC224", "returning references is not supported", d.returnType.span);
      if (d.kind === "python" || (this.target === "python" && !d.name.startsWith("_"))) {
        for (const t of [...fn.params.map(p => p.type), fn.ret]) {
          if (!pythonType(t)) this.err("LYC229", "Python boundary requires scalars, strings, or fixed numeric arrays", d.span);
        }
      }
      const names = new Set<string>();
      for (const param of d.params) {
        if (names.has(param.name)) this.err("LYC216", `duplicate parameter '${param.name}'`, param.span);
        names.add(param.name);
      }
    }
    const main = this.fns.get("main");
    if (this.target === "native" && (!main || main.pythonModule)) this.err("LYC214", "program must define 'main() -> i32'", this.ast.span, "add: def main() -> i32:");
  ...
  private inferExpr(expr: Expr, expected: LycaType | null): LycaType {
    switch (expr.kind) {
      case "int": {
        const ty =
        ...
        if (overflow) this.err("LYC201", `integer literal does not fit in ${ty.kind}`, expr.span);
        return ty;
      }
      case "float": {
        const ty: LycaType = expected?.kind === "f32" ? expected : { kind: "f64" };
        const n = Number(expr.raw);
        if (!Number.isFinite(ty.kind === "f32" ? Math.fround(n) : n)) this.err("LYC201", "float literal out of range", expr.span);
        return ty;
      }
      case "bool":
        return { kind: "bool" };
      ...
      case "unary": {
        if (expr.op === "-" && expr.expr.kind === "int") {
          const ty: LycaType = expected?.kind === "i64" ? expected : { kind: "i32" };
          const max = ty.kind === "i64" ? 9223372036854775808n : 2147483648n;
          if (BigInt(expr.expr.raw) > max) this.err("LYC201", `integer literal does not fit in ${ty.kind}`, expr.span);
      ...
      case "borrow": {
        ...
        if (v.type.kind === "ref") this.err("LYC224", "nested references are not supported", expr.span);
        v.borrowCount++;
  ...
  private checkBinary(expr: Extract<Expr, { kind: "binary" }>, expected: LycaType | null): LycaType {
    ...
    if (cmp.includes(op)) {
      ...
      if (l.kind === "string" || l.kind === "array" || l.kind === "struct" || l.kind === "ref") {
        this.err("LYC225", `cannot compare '${typeName(l)}' with '${op}'`, expr.span);
      }
      return { kind: "bool" };
    }
  ...
  private resolveType(t: TypeAst): LycaType {
    switch (t.kind) {
      case "named":
      ...
      case "array": {
        ...
        if (element.kind === "ref") this.err("LYC224", "arrays cannot contain references", t.span);
        return { kind: "array", element, size: t.size };
      }
      case "ref": {
        const inner = this.resolve(t.inner);
        if (inner.kind === "ref") this.err("LYC224", "nested references are not supported", t.span);
  ...
  private define(name: string, type: LycaType, mut: boolean, span: Span) {
    ...
    if (type.kind === "ref" && mut) this.err("LYC224", "reference bindings must be immutable", span);
  ...
  private requireMutPlace(expr: Expr) {
    if (expr.kind === "name") {
      ...
      if (v.type.kind === "ref") this.err("LYC224", "cannot mutate through a shared reference", expr.span);
    ...
    this.err("LYC105", "assignment requires an owned mutable variable", expr.span);
  }
  ...
  private inferPlace(expr: Expr): LycaType {
    if (expr.kind === "name") {
      const v = this.requireVar(expr.name, expr.span);
      if (v.state === "moved") {
  ...
  private movePlace(expr: Expr, span: Span): void {
    ...
    if (expr.kind === "name") {
      const v = this.requireVar(expr.name, expr.span);
      if (v.type.kind === "ref") this.err("LYC224", "cannot move a non-Copy value through a reference", span);
  ...
  private checkLoopMove(v: VarInfo, span: Span) {
    ...
    if (v.loopDepth < this.loopDepth) this.err("LYC230", "cannot move an outer value in a loop", span, "borrow it instead, or create the value inside the loop");
  }
  ...
  private checkIndex(expr: Expr, size: number) {
    ...
    if (n !== null && (n < 0n || n >= BigInt(size))) this.err("LYC231", "array index out of bounds", expr.span);
  }
...
export function pythonType(t: LycaType): boolean {
